...
\begin{table}[ht]
\centering
\caption{External baseline comparison on the coastal joint-assault scenario (RTX 4090 D). All runs use seed 7. The three baselines are single-pass or few-pass executions evaluated with 30 Monte Carlo simulation runs and are provided as supplementary reference only: their MC run count, iteration budget, and LLM call count are not aligned with the 50-MC, 10-iteration main ablation, so direct numerical comparison with the main settings should be read as indicative rather than strict. Pure and Full rows are from the 10-iteration ablation for reference. All settings use default HOPE parameters ($\theta=0.5$, $\epsilon=0.02$).}
...
\begin{table}[ht]
\centering
\caption{Multi-seed replication (RTX 4090 D): Pure LLM vs.\ Full pipeline across five seeds. All runs use 10 iterations and 50 Monte Carlo simulation runs on the coastal joint-assault scenario, with default HOPE parameters ($\theta=0.5$, $\epsilon=0.02$). The seed-7 row is an independent rerun within this multi-seed suite and therefore differs slightly from the single-scenario ablation (Table~\ref{tab:ablation-main}, seed 7) because of LLM sampling variance ($temperature=0.1$); both runs share the identical configuration and are reported as separate executions.}
\label{tab:multiseed}
\footnotesize
