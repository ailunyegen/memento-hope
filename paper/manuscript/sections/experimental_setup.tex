...
\item \textbf{Full}: memory, Hope, and reflection are all enabled, with HOPE Stochastic Differential Equation (SDE) parameters set to $\theta=0.9$ (reversion coefficient) and $\epsilon=0.005$ (noise strength). These values were selected based on the hyperparameter sensitivity analysis (Section~\ref{sec:hyperparam-sensitivity}), which revealed that the original defaults ($\theta=0.5$, $\epsilon=0.02$) sit near the performance minimum of a U-shaped sensitivity curve.
\end{itemize}
